...
namespace ThingsToKnowAboutServiceBus.FunctionApp_FIFO_SingletonWithoutSessions
{
    public class QueueProcessor
    {
        ...
        public async Task Run([ServiceBusTrigger("fifosingletonqueue", Connection = "ServiceBusConnectionString")]ServiceBusReceivedMessage message, ServiceBusMessageActions messageActions, ServiceBusClient client,
            ...
            [SignalR(HubName = "LogStream")] IAsyncCollector<SignalRMessage> signalRMessages, ILogger log)
        {
            ...
            if (message.DeliveryCount == 1 && message.MessageId.Equals("5"))
            {
                ...
                await signalRMessages.AddAsync(
                    ...
                    {
                        Target = "newMessage",
                        Arguments = new[] { $"Crashed on message: {message.MessageId}; Enqueue time: {message.EnqueuedTime.ToString("hh:mm:ss.fff tt")}; will retry..." }
                    });
                Environment.Exit(-1);
            }
